...
class REPL
  ...
  FUNCTION_SYMBOL = 'fn*'
  QUOTE_SYMBOL = 'quote'
  QUASIQUOTE_SYMBOL = 'quasiquote'
  QUASIQUOTEEXPAND_SYMBOL = 'quasiquoteexpand'
  ...
  def self.evals(ast, env)
    loop do
      if !ast.is_a?(MalListType)
        ...
        when QUOTE_SYMBOL
          return ast.data[1]
        when QUASIQUOTE_SYMBOL
          ast = quasiquote(ast.data[1])
        when QUASIQUOTEEXPAND_SYMBOL
  ...
  def self.quasiquote(ast)
    case ast 
    when MalListType
      if ast.data.length > 0 && ast.data[0].data == 'unquote'
        return ast.data[1]
      else
        def self.inner_quasiquote(ast)
          ...
          elt = ast.data[0]
          if elt.is_a?(MalListType) && elt.data.length > 0 && elt.data[0].data == 'splice-unquote'
